...
**Learning Objectives**:
- Implementing a new agent tool following the `IAgentTool` pattern
...
- `SearchTransactionsTool` registered
...
| **Dependencies** | `BudgetTrackerContext`, `ISemanticSearchService`, `IChatClient` | `IChatClient`, `IToolRegistry` |
...
**Files modified:**
- `src/BudgetTracker.Api/Features/Intelligence/Query/QueryAssistantService.cs` — rewrite internals
...
*Create an aggregation tool that complements the existing search tool.*
...
- Defaults to expenses only (`includeIncome: false`)
- Ordered by spending amount (biggest expenses first)

---

## Step 054.3: Register the New Tool
...
builder.Services.AddScoped<IAgentTool, GetCategorySpendingTool>();  // Add this line
builder.Services.AddScoped<IToolRegistry, ToolRegistry>();
```

Because `ToolRegistry` receives `IEnumerable<IAgentTool>`, the new tool is automatically discovered — no changes needed to the registry itself.

---

## Step 054.4: Update Constructor Dependencies

*Replace database and search dependencies with the tool registry.*

The RAG version needed `BudgetTrackerContext` and `ISemanticSearchService` to manually query data. The agentic version delegates data access to tools via `IToolRegistry`.
...
- Added `IToolRegistry` — provides tools for the agent loop
- `IChatClient` stays — still needed for LLM calls

---

## Step 054.5: Replace ProcessQueryAsync with Agent Loop
...
## Step 054.6: Add ExecuteToolCallsAsync

*Same pattern as `RecommendationAgent.ExecuteToolCallsAsync` — look up the tool, serialize arguments, execute, add result to messages.*
...
1. Look up the tool by name via `_toolRegistry.GetTool()`
2. Serialize `toolCall.Arguments` to `JsonElement`
3. Call `tool.ExecuteAsync(userId, arguments)`
4. Wrap the result in `FunctionResultContent` and add to messages

---

## Step 054.7: Write the System Prompt
...
## Step 054.8: Update Response Parsing
...
### 054.9.2: Try Different Query Types
...
### 054.9.3: Monitor Agent Iterations
...
1. **Adding `GetCategorySpendingTool`**: Aggregation complements semantic search
2. **Swapping dependencies**: `BudgetTrackerContext` + `ISemanticSearchService` → `IToolRegistry`
3. **Replacing the flow**: DB query + semantic search + single LLM call → multi-turn agent loop
4. **Combining tools**: The agent can search, aggregate, or do both depending on the question
5. **Preserving the interface**: `IQueryAssistantService`, endpoint, and frontend unchanged

**Key takeaway**: The tool infrastructure from task 053 is composable. Adding a new tool (`GetCategorySpendingTool`) required zero changes to the agent loop — it's automatically discovered via `IToolRegistry`. The query assistant now has richer capabilities while keeping the same external interface.
